...
Real Random::Density(Real) const {
  Throw(Logic_error("density function not defined"));
  return 0.0;
}
...
{
  if (!seed) Throw(Logic_error("Random number generator not initialised"));
...
{
  if (s >= 1.0 || s <= 0.0) Throw(Logic_error("Newran: seed out of range"));
...
ChiSq1::ChiSq1(Real d)
    : Normal()  // chisquare with 1 df
{
  deltasq = d;
  delta = sqrt(d);
}
...
Real Pareto::Next() { return pow(Random::Next(), RS); }

ExtReal Pareto::Mean() const {
  if (Shape > 1)
    return Shape / (Shape - 1.0);
  else
    return PlusInfinity;
}
...
ExtReal Pareto::Variance() const {
  if (Shape > 2)
    return Shape / (square(Shape - 1.0)) / (Shape - 2.0);
  else
    return PlusInfinity;
}
...
Poisson1::Poisson1(Real mux)
    : AsymGen(mux)  // Constructor
{
  mu = mux;
  ln_mu = log(mu);
}
...
Real Binomial1::Density(Real x) const  // Binomial density function
{
  ...
  double l = ln_n_fac - ln_gamma(ix + 1) - ln_gamma(n - ix + 1) + ix * ln_p +
             (n - ix) * ln_q;
  return (l < -40.0) ? 0.0 : exp(l);
}
...
Binomial2::Binomial2(int nx, Real px) {
  ...
  int m = (nx >= 40) ? 39 : nx;
  for (i = k + 1; i <= m; i++)
    probs[i] = probs[i - 1] * px * (nx - i + 1) / qx / i;
  for (i = k - 1; i >= 0; i--)
    probs[i] = probs[i + 1] * qx * (i + 1) / px / (nx - i);
...
Poisson::Poisson(Real mu) {
  if (mu <= 8.0)
    method = new Poisson2(mu);
  else
    method = new Poisson1(mu);
  if (!method) ErrorNoSpace();
}

Binomial::Binomial(int nx, Real px) {
  if (nx < 40 || nx * px <= 8.0)
    method = new Binomial2(nx, px);
  else
    method = new Binomial1(nx, px);
  if (!method) ErrorNoSpace();
}
...
Gamma2::Gamma2(Real alphax)
    : AsymGen(alphax - 1.0)  // constructor (Real=shape)
{
  alpha = alphax;
...
{
  if (alpha < 1.0)
    method = new Gamma1(alpha);
  else if (alpha == 1.0)
    method = new Exponential();
  else
    method = new Gamma2(alpha);
  if (!method) ErrorNoSpace();
}
...
{
  int i = (int)(n * Random::Next());
  if (Random::Next() < p[i]) i = ialt[i];
...
AddedSelectedRandom& operator+(SelectedRandom& rv1, SelectedRandom& rv2) {
  AddedSelectedRandom* r = new AddedSelectedRandom(rv1, rv2);
  if (!r) ErrorNoSpace();
  return *r;
}

AddedSelectedRandom& operator+(AddedSelectedRandom& rv1, SelectedRandom& rv2) {
  AddedSelectedRandom* r = new AddedSelectedRandom(rv1, rv2);
  if (!r) ErrorNoSpace();
  return *r;
}

AddedSelectedRandom& operator+(SelectedRandom& rv1, AddedSelectedRandom& rv2) {
  AddedSelectedRandom* r = new AddedSelectedRandom(rv1, rv2);
  if (!r) ErrorNoSpace();
  return *r;
}
...
const char* Random::Name() const { return "Random"; }
const char* Uniform::Name() const { return "Uniform"; }
const char* Constant::Name() const { return "Constant"; }
const char* PosGen::Name() const { return "PosGen"; }
const char* SymGen::Name() const { return "SymGen"; }
const char* AsymGen::Name() const { return "AsymGen"; }
const char* PosGenX::Name() const { return "PosGenX"; }
const char* SymGenX::Name() const { return "SymGenX"; }
const char* AsymGenX::Name() const { return "AsymGenX"; }
const char* Normal::Name() const { return "Normal"; }
const char* ChiSq::Name() const { return "ChiSq"; }
const char* Cauchy::Name() const { return "Cauchy"; }
const char* Exponential::Name() const { return "Exponential"; }
const char* Poisson::Name() const { return "Poisson"; }
const char* Binomial::Name() const { return "Binomial"; }
const char* NegativeBinomial::Name() const { return "NegativeBinomial"; }
const char* Gamma::Name() const { return "Gamma"; }
const char* Pareto::Name() const { return "Pareto"; }
const char* DiscreteGen::Name() const { return "DiscreteGen"; }
const char* SumRandom::Name() const { return "SumRandom"; }
const char* MixedRandom::Name() const { return "MixedRandom"; }
