...
#define WANT_STREAM  // rng_include.h will get stream fns

#include <string.h>  // C string library

#include "rng_include.h"  // include standard files
...
Exception::Exception(const char* a_what) {
  ...
  if (!what_error)  // make space for exception message
  {
    ...
    {
      LastOne = 0;
      what_error = const_cast<char*>("No heap space for exception message\n");
    }
  }
...
Janitor::~Janitor() {
  ...
  if (OnStack) {
    ...
    if (this == lastjan)
      ...
      Throw(
          Exception("Cannot resolve memory linked list\nSee notes in "
                    "myexcept.cpp for details\n"));
...
Runtime_error::Runtime_error(const char* a_what) : Exception() {
  Select = Exception::Select;
