...
void test3(int n) {
...

  {
    ...
    for (int i = 0; i < n; i++) {
      Real x = X.Next();
      data[i] = 0.20 * NormalDF(x) + 0.35 * NormalDF((x - 1.1) / 2.5) +
                0.45 * NormalDF(x - 2.3);
    }
    cout << X.Name() << ":   " << KS(data, n) << endl;
  }
...
static void MyQuickSortAscending(Real* first, Real* last, int depth) {
  for (;;) {
    ...
    if (depth++ > MaxDepth) Throw(Exception("QuickSortAscending fails"));
...
Real NormalDF(Real x) {
  ...
  t = (0.319381530 +
       (-0.356563782 +
        (1.781477937 + (-1.821255978 + 1.330274429 * t) * t) * t) *
           t) *
      t;
...
void TestNegativeBinomial(Real NX, Real P, int n) {
  ...
  Real var = mean * Q;
  int N =
      (int)(20 + mean + 100 * sqrt(var));  // set upper bound
