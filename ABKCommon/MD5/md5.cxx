...
MD5::MD5() : _isFinalized(false) { _initState(); }

MD5::MD5(const char *input, unsigned length, bool lastUpdate)
    : _isFinalized(false) {
  _initState();
...
void MD5::_update(const unsigned char *input, unsigned inputLen) {
  ...
  if ((_count[0] += ((unsigned)inputLen << 3)) < ((unsigned)inputLen << 3))
    _count[1]++;
  ...
  if (inputLen >= partLen) {
    ...
    for (i = partLen; i + 63 < inputLen; i += 64) _transform(&input[i]);
