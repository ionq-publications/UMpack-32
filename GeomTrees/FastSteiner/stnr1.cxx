...
void stnr1_package_init(long max_n_points, long flags) {
  ...
  if (max_n_triples < max1) {
    max_n_triples = max1;
    triple =
        (Triple*)realloc((void*)triple, (size_t)max_n_triples * sizeof(Triple));
  }
  if (max_n_edges < max_n_points) {
    ...
    sorted_stnr =
        (long*)realloc((void*)sorted_stnr, (size_t)max_n_points * sizeof(long));
...
void stnr1_package_done() {
  ...
  if (sorted_stnr) {
    free((void*)sorted_stnr);
    sorted_stnr = (long*)NULL;
  }
...
#ifdef DEBUG
static int compare_triples_by_gain(const void* t1, const void* t2) {
  double diff = ((Triple*)t2)->gain - ((Triple*)t1)->gain;

  return (diff < 0 ? -1
                   : (diff > 0 ? 1 : ((Triple*)t1)->id - ((Triple*)t2)->id));
}
#endif

static bool std_compare_triples_by_gain(const Triple& t1, const Triple& t2) {
  double diff = t2.gain - t1.gain;

  return (diff < 0 ? true : (diff > 0 ? false : t1.id < t2.id));
}
...
*/
static double apply_triples(long* n_points, long max_n_points, Point* pt,
                            long* n_triples, long n_edges, long* round_new_stnr,
                            long max_stnr_per_round, long* phase_new_stnr,
...
/*
 */
double stnr1(long* n_points, long max_n_points, Point* pt, long* parent,
             long* n_rounds, long max_rounds, long* n_phases,
             long max_phases_per_round, long max_stnr_per_round,
             long max_stnr_per_phase, double cut_off, long flags) {
  long i;
  ...
  */
  ...
  std::sort(sorted_terms, sorted_terms + (*n_points), std_compare_points);
  ...
  do {
    ...
    for (i = 1; i < *n_points; i++) {
      printf("%ld %ld (%f,%f) -> (%f,%f)   %f\n", i, parent[i], pt[i].x,
             pt[i].y, pt[parent[i]].x, pt[parent[i]].y,
    ...
    ***************************************************************/
    ...
    do {
      ...
      std::sort(triple, triple + n_triples, std_compare_triples_by_gain);

#ifdef DEBUG
      assert(is_sorted(triple, n_triples, sizeof(Triple),
                       compare_triples_by_gain));
#endif

      len -= apply_triples(n_points, max_n_points, pt, &n_triples, n_edges,
                           &round_new_stnr, max_stnr_per_round, &phase_new_stnr,
                           max_stnr_per_phase);
      ...
      */
      if (n_triples > 0) {
        ...
        remove_negative_gain_triples(&n_triples, triple, edge);
             ...
             (round_new_stnr < max_stnr_per_round) && (n_triples > 0));
