...
int main(int argc, char** argv) {
  ...
  */
  for (i = 1; i < argc; i++) {
    ...
    } else if (!strncmp(argv[i], "-max_phases_per_round",
                        strlen("-max_phases_per_round"))) {
      max_phases_per_round = atol(argv[++i]);
    } else if (!strncmp(argv[i], "-max_stnr_per_round",
                        strlen("-max_stnr_per_round"))) {
      max_stnr_per_round = atol(argv[++i]);
    } else if (!strncmp(argv[i], "-max_stnr_per_phase",
                        strlen("-max_stnr_per_phase"))) {
      max_stnr_per_phase = atol(argv[++i]);
    } else if (!strncmp(argv[i], "-cut_off", strlen("-cut_off"))) {
      cut_off = atof(argv[++i]);
    } else if (!strncmp(argv[i], "-timed_stnr_runs",
                        strlen("-timed_stnr_runs"))) {
      timed_stnr_runs = atol(argv[++i]);
    } else if (!strncmp(argv[i], "-timed_mst_runs",
                        strlen("-timed_mst_runs"))) {
