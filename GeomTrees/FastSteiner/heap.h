...
inline void heap_insert(long p, double key) {
...
inline void heap_decrease_key(long p, double new_key) {
  long k; /* hole in the heap     */
  long j; /* parent of the hole   */
  long q; /* heap_elt(j)          */
...
inline long heap_delete_min() {
  ...
  double l_key; /* key of last point    */
  ...
  j = 2;
  while (j <= _heap_size) {
    if (heap_key(heap_elt(j)) > heap_key(heap_elt(j + 1))) j++;
