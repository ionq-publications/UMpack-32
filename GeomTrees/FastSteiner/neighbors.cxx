...
long octant(Point from, Point to);
...
#ifdef DEBUG
static int compare_shear(const void* i, const void* j) {
...
static bool std_compare_shear(const long& i, const long& j) {
  const Point& pi = sheared[i];
  const Point& pj = sheared[j];

  if (pi.x < pj.x) return true;
  if (pi.x > pj.x) return false;
  return (pi.y < pj.y);
}
...
static void ne_sw_combine(long left, long mid, long right, Point* pt,
                          long* sorted, long* aux, long oct, nn_array* nn) {
  long i, j, k;
  long i1;
  long i2;
...
void ne_sw_nearest_neighbors(long left, long right, Point* pt, long* sorted,
                             long* aux, long oct, nn_array* nn) {
  long mid;
...
void dq_nearest_neighbors(long n, Point* pt, nn_array* nn) {
  ...
  double rectilinear_shear[4][4] = {
      {1, -1, 0, 2}, {2, 0, -1, 1}, {1, 1, -2, 0}, {0, 2, -1, -1}};

  double octilinear_shear[4][4] = {{1, -1, 0, SQRT_TWO},
                                   {SQRT_TWO, 0, -1, 1},
                                   {1, 1, -SQRT_TWO, 0},
                                   {0, SQRT_TWO, -1, -1}};
  ...
  for (oct = 0; oct < 4; oct++) {
    ...
    std::sort(sorted, sorted + n, std_compare_shear);

#ifdef DEBUG
    assert(is_sorted(sorted, n, sizeof(long), compare_shear));
#endif
